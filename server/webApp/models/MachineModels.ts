...
class MachineModel {
  async getMachinesForUser(userId: string): Promise<SupplierTypes.Machine[]> {
    ...
    const items = await conn.query(`
      ...
      WHERE M_ID in ("${machines.map(({MACHINE_ID}: {MACHINE_ID: string}) => MACHINE_ID).join('","')}")
    `);

    return aggregateMachineItems(machines, items);
  }
}


const aggregateMachineItems = (machines: any[], items: any[]): SupplierTypes.Machine[] => {
  const groupedItems = _.groupBy(items, ({M_ID}) => M_ID);
  console.log(groupedItems)
  return machines.map(({MACHINE_ID, FLOOR, MACHINE_NUMBER, OWNER_ID}: any) => ({
